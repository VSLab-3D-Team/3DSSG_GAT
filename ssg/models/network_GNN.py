...
class MSG_FAN_With_CLIP(MessagePassing):
    def __init__(self,
                 dim_node: int, 
                 dim_edge: int, 
                 dim_atten: int,
        ...
        self.proj_q = build_mlp([dim_node, dim_node], do_bn=use_bn)
        self.proj_k = build_mlp([dim_edge, dim_edge], do_bn=use_bn)
        self.proj_v = build_mlp([dim_node, dim_atten], do_bn=use_bn)
        
        self.clip_proj = build_mlp([dim_clip, dim_edge], do_bn=use_bn)
    ...
    def forward(self, data):
        ...
        projected_text_embeddings = self.clip_proj(text_embeddings)
        
        node_feature, edge_feature, probs_3d, probs_text, kl_divs = self.propagate(
            edge_index, x=x, edge_feature=edge_feature, 
            text_embedding=projected_text_embeddings, x_ori=x)
    ...
    def message(self, x_i, x_j, edge_feature, text_embedding):
        ...
        clip_k = text_embedding.view(num_edges, self.dim_edge_proj, self.num_head)  # ρ_clip
        
        att_3d = self.nn_att_3d(torch.cat([q, k], dim=1))  # MLP_att(ρ_i || ρ_ij)
        prob_3d = F.softmax(att_3d/self.temperature, dim=1)
        prob_3d = self.dropout(prob_3d)
        value_3d = prob_3d.reshape_as(v) * v
        
        if self.use_text_attention:
            att_text = self.nn_att_text(torch.cat([q, k, clip_k], dim=1))  # MLP_att(ρ_i || ρ_ij || ρ_clip)
            prob_text = F.softmax(att_text/self.temperature, dim=1)
        ...
        kl_div = F.kl_div(
            F.log_softmax(prob_3d, dim=1),
            F.softmax(prob_text, dim=1),
            reduction='none'
        ).sum(dim=1).mean()
...
class GraphEdgeAttenNetworkLayers(nn.Module):
    """ A sequence of scene graph convolution layers with CLIP text embedding """
    ...
    def __init__(self, **kwargs):
        ...
        if 'DROP_OUT_ATTEN' in kwargs:
            self.drop_out = nn.Dropout(kwargs['DROP_OUT_ATTEN'])
        
        self.node_class_names = kwargs.get('node_class_names', [])
        self.edge_class_names = kwargs.get('edge_class_names', [])
        
        use_text_attention = kwargs.get('use_text_attention', True)
        
        for _ in range(self.num_layers):
            self.gconvs.append(MSG_FAN_With_CLIP(
                dim_node=kwargs['dim_node'],
                dim_edge=kwargs['dim_edge'],
                dim_atten=kwargs['dim_atten'],
                dim_clip=kwargs.get('dim_clip', 512),
